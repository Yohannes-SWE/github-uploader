...
from universal_deployer import (
    UniversalDeployer,
    DeploymentConfig,
    ServiceType,
    create_provider,
...
def get_supported_providers():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "providers": providers, "provider_info": provider_info}
        )
...
def configure_provider(provider_name):
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify({"success": False, "error": f"Configuration failed: {str(e)}"}),
                400,
            )
...
def deploy_application():
    ...
    try:
        ...
        if not repo_url or not project_name:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "repo_url and project_name are required",
                    }
                ),
                400,
            )
        ...
        for provider in providers:
            try:
                result = deployer.deploy_to_provider(
                    provider, deployment_config, repo_url
                )
                results[provider] = {
...
def get_deployment_history():
    ...
    try:
        ...
        return jsonify({"success": True, "deployments": deployment_history})
            ...
            if result["service_id"] and result["deployment_id"]:
                try:
                    status = deployer.get_deployment_status(
                        provider, result["service_id"], result["deployment_id"]
                    )
                    updated_results[provider] = {**result, "status": status.value}
                except Exception as e:
                    logger.error(f"Error getting status for {provider}: {e}")
                    updated_results[provider] = result

        return jsonify(
            {"success": True, "deployment": {**deployment, "results": updated_results}}
        )
...
def add_custom_domain(provider_name):
    ...
    try:
        ...
        if not service_id or not domain:
            return (
                jsonify(
                    {"success": False, "error": "service_id and domain are required"}
                ),
                400,
            )
